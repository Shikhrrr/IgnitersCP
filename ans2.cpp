...
// There is a problem with the first test case given in the question
// Taking the bitwise AND of any two numbers gives a result that is either equal to or 
// lesser than the smallest number. This means that if the size of the subarray is more
// than one, we will anyway won't get the largest answer. Hence the ideal subarray size
// is one, that is, every element itself. To get the maximum bitwies AND, we just need 
// to find the largest number. in the first test case '1 2 3', the answer should be 3
// and not 0. 
// This code works to find the largest element, which is the required answer.
