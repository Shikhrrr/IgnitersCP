...
// There is a problem with the third test case in the given question.
// for d = 3, ans should be 55 not 35.
// this is how -> 35 as a product of primes is 5*7. the first prime number greater than 3 is 5, but then the 
// the other required number should be atleast 5+3 = 8. the smallest prime greater than or equal to 8 is 11
// so the answer becomes 55, not 35.
...
void solve(vl sieve) {
    ...
    while (p1 < sieve.size() and sieve[p1] <= d) {
        p1++;
    }

    while (pd < sieve.size() and sieve[pd] < sieve[p1] + d) {
        pd++;
    }

    cout << sieve[p1]*sieve[pd] << ln;
}
